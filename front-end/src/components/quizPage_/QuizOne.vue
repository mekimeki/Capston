            ...
            <v-tab-item v-for="n in 7" :key="n">
                <v-card justify-center>
                            ...
                            <v-flex v-if="n>=2" xs3 sm12>
                                <v-layout justify-space-around row wrap>
                                    <v-flex v-for="n in 4" :key="n" md5>
                                        <v-btn large block color="orange" v-on:click="select=(n-1)" value="check">{{ n + ". " + example[n-1].word}}</v-btn>
                                    </v-flex>
                                </v-layout>
                            </v-flex>
                        </v-layout>
                    ...
                    <v-card-actions>
                        ...
                        <v-btn v-if="1<n&&n<6" color="red lighten-1" flat="flat" v-on:click="next(), getQuest(), checkAns()">{{ button }}</v-btn>

                        <v-btn v-if="n==1" color="red lighten-1" flat="flat" v-on:click="next(), getQuest()">{{ button }}</v-btn>

                        <v-btn v-if="n==6" color="red lighten-1" flat="flat" v-on:click="next(),checkAns()">{{ button }}</v-btn>

                        <v-btn v-if="n==7" color="red lighten-1" flat="flat" v-on:click.stop="dialog = false, postQuest()">{{ close }}</v-btn>
                    </v-card-actions>
                </v-card>
            </v-tab-item>
        </v-tabs>
...
export default {
    ...
    methods: {
        ...
        getQuest() {
            // this.quest_actions('').then(result => {
            //     console.log('quiz quest sucess', result);
            //     const back = result;
            ...
            // });
            const baseURI = "http://172.26.2.194/api/quiz";
            axios
                .get(baseURI)
                .then(res => {
                    var back = res.data;
        ...
        //FormData()方式
        postQuest() {
            // this.quest_post_actions(this.score).then(result => {
            ...
            axios.get("http://172.26.2.194/get-token").then(response => {
                console.log('quiz post sucess')
                if (response.data) {
                    form.append("_token", response.data);
                    axios
                        .post("http://172.26.2.194/api/quiz", form)
                        .then(response => {
