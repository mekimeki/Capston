...
<v-container pa-5>
    ...
    <v-layout wrap row fill-height>
        ...
        <v-flex class="px-1 py-4 pt-5" v-for="(name,i) in movies" :key="(name,i)" xs12 sm4 md2 flexbox>
            <v-card router :to="{name:'v-video'}" color="white" flat>
                <v-img :src="`http://192.168.0.19/movie/`+`${Math.floor(Math.random() * 47)}.jpg`" aspect-ratio="1.5"></v-img>
                <div>
