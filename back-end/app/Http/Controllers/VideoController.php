...
class VideoController extends Controller
{
    ...
    public function __construct()
    {
        ...
        $GLOBALS['ffprobe'] = \FFMpeg\FFProbe::create();
    }
    ...
    {
        $ff = $GLOBALS['ffmpeg'];
        $ffprobe = $GLOBALS['ffprobe'];
        $videoPath = public_path('audio\\test.mp4');

        $video = $ff->open($videoPath);
        $duration = $ffprobe->format($videoPath)->get('duration');

        $j = 1;
        for ($i = 0; $i <= $duration; $i = $i + ($duration / 10)) {
            $frame = $video->frame(\FFMpeg\Coordinate\TimeCode::fromSeconds($i));
            $frame->save('test' . $j . '.jpg');
            $j++;
        }
